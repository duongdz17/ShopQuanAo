							...
							<div class="card">
								<div class="card-body p-4">
									<a class="btn btn-danger mb-2"
										href="addUser.jsp">Thêm mới tài khoản</a>
									...
									<table class="table" id="myTable">
										...
										%>
												<tr>
													<td><%=user.getHoten() %></td>
													<td><%=user.getEmail() %></td>
													<th><%=user.getSodienthoai() %></th>
													<td><%=user.getDiachi() %></td>
													<td><%=user.getNgaysinh() %></td>
													<td><%=user.getMaquyentruycap() %></td>
													<td hidden="">
													<img style="width: 100px; height: 50px" alt="" src="${url}/images/profile/<%=user.getAnhnguoidung()%>">
													</td>
													<td>
													<a href="updateUser.jsp?userid=<%=user.getUserid() %>">
